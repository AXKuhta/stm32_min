...
void bias_init() {
	...
	// OA2 bias network
	HAL_GPIO_Init(GPIOG, &(GPIO_InitTypeDef){ .Pin = GPIO_PIN_0, .Mode = GPIO_MODE_OUTPUT_PP, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FAST });
	HAL_GPIO_Init(GPIOF, &(GPIO_InitTypeDef){ .Pin = GPIO_PIN_13, .Mode = GPIO_MODE_OUTPUT_PP, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FAST });
	HAL_GPIO_Init(GPIOF, &(GPIO_InitTypeDef){ .Pin = GPIO_PIN_15, .Mode = GPIO_MODE_INPUT, .Pull = GPIO_NOPULL, .Speed = GPIO_SPEED_FAST });
...
void opamp2_init() {
	...
	opamp2 = (OPAMP_HandleTypeDef) {
		...
		.Init = {
			...
			.PgaConnect = OPAMP_PGA_CONNECT_INVERTINGINPUT_IO0,
