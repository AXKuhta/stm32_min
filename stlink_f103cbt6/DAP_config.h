...
#define CPU_CLOCK               (HAL_RCC_GetSysClockFreq())      ///< Specifies the CPU Clock in Hz.
...
#define PIN_SWDIO GPIOB, 14 // Also PB12
#define PIN_SWCLK GPIOB, 13 //
...
static __inline void PORT_SWD_SETUP (void) {
  initPin_PP(PIN_SWCLK);
  initPin_PP(PIN_SWDIO);
}
...
static __inline void PORT_OFF (void) {
  initPin_InputF(PIN_SWCLK);
  initPin_InputF(PIN_SWDIO); // Pullup needed?
}
...
static __forceinline uint32_t PIN_SWDIO_TMS_IN  (void) {
  return (readPin(PIN_SWDIO) == 1) ? 1 : 0;
}
...
static __forceinline void     PIN_SWDIO_OUT     (uint32_t bit) {
  setPin_PP ( PIN_SWDIO, (bit & 1) ? ON : OFF );
}
